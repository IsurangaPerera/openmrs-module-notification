...
@Resource(name = RestConstants.VERSION_1
		+ "/notification", supportedClass = Subscription.class, supportedOpenmrsVersions = {"1.9.*", "1.10.*", "1.11.*",
				"1.12.*", "2.0.*", "2.1.*"})
public class SubscriptionResource extends DelegatingCrudResource<Subscription> {
	...
	protected PageableResult doGetAll(RequestContext context) throws ResponseException {
		List<Subscription> subscriptionList = Context.getService(SubscriptionManagementService.class)
				.getAllSubscriptions();
